 ...
 * 
 * Run: npx tsx scripts/seed-officer.ts
 * 
 * This creates:
 * 1. Firebase Auth user (for authentication)
 * 2. governmentOfficers document (for RBAC in firestore.rules)
 */
...
const serviceAccountPath = resolve(process.cwd(), '..', 'api-server', 'service-account.json');
...
async function main() {
  ...
  for (const officer of OFFICERS) {
    try {
      ...
      // 2. Create governmentOfficers document
